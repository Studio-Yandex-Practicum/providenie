...
def checking_not_digits(data: str) -> bool:
    ...
    if (
        all(
            x.isalpha() or x.isspace() or x in constants.SPEC_SYM for x in data
        )
        and data != " "
        and len(data) < 100
    ):
...
def checking_email(data: str) -> bool:
    """Проверка email."""
    return bool(re.match(constants.REGEX_EMAIL, data))
...
def checking_birthday(data: str) -> bool:
    ...
    if bool(re.match(constants.REGEX_BIRTHDAY, data)):
        ...
        if int(date_split_list[1]) < 1 or int(date_split_list[1]) > 12:
            flag = False
        if (
            int(date_split_list[2]) < 1950
            or int(date_split_list[2]) > date_now.year
        ):
            flag = False
        if (
            int(date_split_list[2]) == date_now.year
            and int(date_split_list[1]) >= date_now.month
            and int(date_split_list[0]) > date_now.day
        ):
            flag = False
    else:
        flag = False
