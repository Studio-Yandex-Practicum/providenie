...
async def create_users(
    ...
    group_id: Optional[List[int]] = Form(
        None
    ),  # Переименовал из groups в group_id под шаблон  # noqa: E501
    session: AsyncSession = Depends(get_async_session),
) -> HTMLResponse:
    ...
    user = UserCreate(
        ...
        is_active=is_active,
        groups=group_id,
    )  # Переименовал из groups в group_id под шаблон
    user = await crud_user.create(user, session)
    groups = await session.execute(
        select(Group).filter(Group.id.in_(group_id))
    )  # Переименовал из groups в group_id под шаблон  # noqa: E501
    groups = groups.scalars().all()
    return templates.TemplateResponse(
        "user_created.html", {"request": request, "user": user, "groups": groups}
    )
...
async def get_user_edit_form(
    ...
    is_admin: Optional[bool] = Form(False),
    password: Optional[str] = Form(None),
    group_id: Optional[List[int]] = Query(
        None
    ),  # Переименовал из groups в group_id под шаблон  # noqa: E501
    session: AsyncSession = Depends(get_async_session),
) -> HTMLResponse:
    """Endpoint to get editing form an existing user."""
    existing_user = await crud_user.get_one_by_attributes({"id": user_id}, session)
    ...
    groups = await crud_group.get_all_objs(session)
    return templates.TemplateResponse(
        "edit_user.html", {"request": request, "user": user, "groups": groups}
    )
...
async def edit_user(
    ...
    if group_id:  # Переименовал из groups в group_id под шаблон
        groups = await session.execute(
            select(Group).filter(Group.id.in_(user_scheme.groups))
        )
        groups = groups.unique().scalars().all()
    return templates.TemplateResponse(
        "user_updated.html",
